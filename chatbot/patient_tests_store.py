"""Ground-truth Q/A tests for the patient chatbot.

- Cases: a flat list at ``patient_tests/cases.json``.
- Runs: one latest run per prompt-version at ``patient_tests/runs/v{vid}.json``.
  Re-running a version overwrites its run file.
"""
...
ROOT = Path(__file__).parent / "patient_tests"
...
def add_case(question: str, gt_answer: str) -> dict:
    cases = list_cases()
...
def update_case(case_id: int, question: str, gt_answer: str) -> dict | None:
    cases = list_cases()
    for c in cases:
        if c["id"] == case_id:
...
def all_run_summaries() -> dict[int, dict]:
    """Map version_id → {avg_score, ran_at, n}."""
    _ensure()
    out: dict[int, dict] = {}
